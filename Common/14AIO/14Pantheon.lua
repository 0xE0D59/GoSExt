...
function Pantheon:LoadMenu()
    ...
    self.tyMenu.Setting:MenuElement(
                {id = "q1hitchance", name = "Q1 HitChance", value = 1, drop = {"Normal", "High", "Immobile"}}
            )
	self.tyMenu.Setting:MenuElement(
                {id = "q2hitchance", name = "Q2 HitChance", value = 1, drop = {"Normal", "High", "Immobile"}}
            )
...
function Pantheon:CastQ1(target)
    ...
    and orbwalker:CanMove() and GetDistanceSquared(myHero.pos,target.pos) < self.Q1.Range ^2 then
        ...
        if Pred:CanHit(self.tyMenu.Setting.q1hitchance:Value() + 1) then
...
function Pantheon:CastQ2(target)
    ...
    and self.Qchannel and Ready(_Q)  and  lastQup + 150 < GetTickCount() then
        ...
        if Pred:CanHit(self.tyMenu.Setting.q2hitchance:Value() + 1)  then
