...
do
    ...
    Menu.potter_regular =
        Menu.potter:MenuElement(
        {
            id = "regular",
            name = "Health Potions",
            value = true,
            leftIcon = "https://ddragon.leagueoflegends.com/cdn/10.23.1/img/item/2003.png"
        }
    )
    Menu.potter_cookies =
        Menu.potter:MenuElement(
        {
            ...
            name = "Biscuit",
            value = true,
            leftIcon = "https://ddragon.leagueoflegends.com/cdn/10.23.1/img/item/2010.png"
        }
    )
    Menu.potter_refill =
        Menu.potter:MenuElement(
        {
            ...
            value = true,
            leftIcon = "https://ddragon.leagueoflegends.com/cdn/10.23.1/img/item/2031.png"
        }
    )
    Menu.potter_corrupt =
        Menu.potter:MenuElement(
        {
            ...
            value = true,
            leftIcon = "https://ddragon.leagueoflegends.com/cdn/10.23.1/img/item/2033.png"
        }
    )
    Menu.potter_health =
        Menu.potter:MenuElement(
        {id = "healthPercent", name = "Use if HP% below:", value = 50, min = 1, max = 100, identifier = "%"}
    )
    ...
    Menu.zhonya_allies =
        Menu.zhonya:MenuElement(
        {id = "allies", name = "Show Allies", value = true, leftIcon = "http://puu.sh/rGoYo/0e0e445743.png"}
    )
    Menu.zhonya_enemies =
        Menu.zhonya:MenuElement(
        {id = "enemies", name = "Show Enemies", value = true, leftIcon = "http://puu.sh/rGoYt/5c99e94d8a.png"}
    )
    Menu.zhonya_player =
        Menu.zhonya:MenuElement(
        {id = "player", name = "Show Me", value = true, leftIcon = "http://puu.sh/rGoYo/0e0e445743.png"}
    )
    Menu.zhonya_size_inner =
        Menu.zhonya:MenuElement({id = "sizeInner", name = "Inner Radius", value = 150, min = 5, max = 600, step = 5})
    Menu.col_inner = Menu.zhonya:MenuElement({id = "col", name = "Inner Color", color = Draw.Color(250, 250, 210, 85)})
    Menu.zhonya_size_outer =
        Menu.zhonya:MenuElement({id = "sizeOuter", name = "Outer Radius", value = 400, min = 5, max = 600, step = 5})
    ...
    Menu.items_profaneuse =
        Menu.items_profane:MenuElement({name = "Use Profane Hydra", id = "profaneuse", value = true})
    Menu.items_profaneusecombo =
        Menu.items_profane:MenuElement({name = "Only in combo", id = "profaneusecombo", value = true})
    Menu.items_profaneusebelow =
        Menu.items_profane:MenuElement({name = "Only if target below 50% HP", id = "profaneusebelow", value = true})
    Menu.items_profaneblacklist =
        Menu.items_profane:MenuElement({name = "Blacklist", id = "profaneblacklist", type = _G.MENU})
    Menu.items_profaneblock =
        Menu.items_profane:MenuElement({name = "Block if spell ready", id = "profaneblock", type = _G.MENU})
    Menu.items_profaneblockq =
        Menu.items_profaneblock:MenuElement({name = "Block if Q ready", id = "profaneblockq", value = false})
    Menu.items_profaneblockw =
        Menu.items_profaneblock:MenuElement({name = "Block if W ready", id = "profaneblockw", value = false})
    Menu.items_profaneblocke =
        Menu.items_profaneblock:MenuElement({name = "Block if E ready", id = "profaneblocke", value = false})
    Menu.items_profaneblockr =
        Menu.items_profaneblock:MenuElement({name = "Block if R ready", id = "profaneblockr", value = false})
    ...
    do
        ...
        ZhonyaTimers.Tick = function()
            if not Menu.zhonya_enabled:Value() or myHero.dead or Game.IsChatOpen() then
                return
            end
            for i = 1, Game.HeroCount() do
                local hero = Game.Hero(i)
        ...
        ZhonyaTimers.Draw = function()
            ...
            for i = 1, Game.HeroCount() do
                local hero = Game.Hero(i)
                if
                    (hero.team == myHero.team and Menu.zhonya_allies:Value() and hero ~= myHero) or
                        (hero.team ~= myHero.team and Menu.zhonya_enemies:Value()) or
                        (hero == myHero and Menu.zhonya_player:Value())
                 then
    ...
    do
        ...
        Potter.Tick = function()
            if not Menu.potter_enabled:Value() or Potter.NextTick > Game.Timer() or myHero.dead or Game.IsChatOpen() then
                return
            end
        ...
        function Potter:ShouldPlayerDrinkPotion()
            return not myHero.dead and myHero.maxHealth ~= 0 and
                (myHero.health / myHero.maxHealth) < Menu.potter_health:Value() / 100
        end
        ...
        function Potter:GetHealingBuff()
            ...
            for i = 0, myHero.buffCount do
                ...
                if
                    buff.type == 14 and buff.duration > 0 and
                        (buff.name == "Item2003" or buff.name == "ItemCrystalFlask" or
                            buff.name == "ItemDarkCrystalFlask" or
                            buff.name == "Item2010")
                 then
                    return buff
                end
        ...
        function Potter:GetDrinkablePotionSlot()
            for _, j in pairs({ITEM_1, ITEM_2, ITEM_3, ITEM_4, ITEM_5, ITEM_6}) do
                -- Regular Pot
                if
                    Menu.potter_regular:Value() and myHero:GetItemData(j).itemID == 2003 and
                        myHero:GetSpellData(j).currentCd == 0
                 then
                    return j
                end
                -- Refillable Pot
                if
                    Menu.potter_refill:Value() and myHero:GetItemData(j).itemID == 2031 and
                        myHero:GetSpellData(j).currentCd == 0 and
                        myHero:GetItemData(j).ammo > 0
                 then
                    return j
                end
                -- Corrupting Pot
                if
                    Menu.potter_corrupt:Value() and myHero:GetItemData(j).itemID == 2033 and
                        myHero:GetSpellData(j).currentCd == 0 and
                        myHero:GetItemData(j).ammo > 0
                 then
                    return j
                ...
                -- Biscuit
                if
                    Menu.potter_cookies:Value() and myHero:GetItemData(j).itemID == 2010 and
                        myHero:GetSpellData(j).currentCd == 0
                 then
                    return j
                end
    ...
    Items.Tick = function()
        if not Menu.items_profaneuse:Value() or Items.NextTick > Game.Timer() or myHero.dead or Game.IsChatOpen() then
            Items.NextTick = Game.Timer() + 0.025
            return
        end
        if Menu.items_profaneusecombo:Value() and not Items:IsCombo() then
            Items.NextTick = Game.Timer() + 0.025
            return
        end
        if not _G.SDK.ItemManager:IsReady(myHero, Items.ProfaneHydraID) then
            Items.NextTick = Game.Timer() + 0.05
            return
        end
        if Menu.items_profaneblockq:Value() and Items:IsSpellReady(_Q) then
            Items.NextTick = Game.Timer() + 0.05
            return
        end
        if Menu.items_profaneblockw:Value() and Items:IsSpellReady(_W) then
            Items.NextTick = Game.Timer() + 0.05
            return
        end
        if Menu.items_profaneblocke:Value() and Items:IsSpellReady(_E) then
            Items.NextTick = Game.Timer() + 0.05
            return
        end
        if Menu.items_profaneblockr:Value() and Items:IsSpellReady(_R) then
            Items.NextTick = Game.Timer() + 0.05
            return
        end
        for i = 1, Game.HeroCount() do
            local hero = Game.Hero(i)
            if
                (hero.team ~= myHero.team) and
                    not Menu.items_profaneblacklist["profaneblacklist_" .. hero.charName]:Value() and
                    hero.valid and
                    hero.isTargetable and
                    hero.alive and
                    hero.visible and
                    hero.networkID and
                    myHero.pos:DistanceTo(hero.pos) <= 425 and
                    hero.health > 0
             then
